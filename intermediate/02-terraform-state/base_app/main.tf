...
locals {
  ...
  instance_name = "${local.naming_prefix}_app" # changed from "${local.naming_prefix}-web" to "${local.naming_prefix}_app"
...
module "networking" {
  ...
  name = local.vpc_name # var.vpc_network_info["vpc_name"] also works
...
# changed aws_instance.web to aws_instance.app
